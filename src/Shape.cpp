...
void Shape::loadMesh(const string &meshName, string *mtlpath, unsigned char *(loadimage)(char const *, int *, int *, int *, int))
{
	...
	else if (shapes.size())
	{
		...
		colBufID = new unsigned int[shapes.size()];
...
void Shape::init()
{
...

	{
		...
		// send color array to the GPU - color uses positions
		glGenBuffers(1, &colBufID[i]);
		glBindBuffer(GL_ARRAY_BUFFER, colBufID[i]);
		glBufferData(GL_ARRAY_BUFFER, posBuf[i].size() * sizeof(float), posBuf[i].data(), GL_STATIC_DRAW);
...
void Shape::draw(const shared_ptr<Program> prog) const
{
...

	{
		...
		// bind position to color buffer for default coloring
